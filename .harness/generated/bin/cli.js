...
// Generated by metaharness — the `dynamo-mcp-agent` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx dynamo-mcp-agent`
...
// "bin" + "files" fields in package.json), so `npx dynamo-mcp-agent` works the moment
...
const HARNESS_NAME = 'dynamo-mcp-agent';

/** `dynamo-mcp-agent init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `dynamo-mcp-agent doctor` — verify the install end-to-end (kernel + host resolve). */
