...
`read-optimization.json` records 40 warmups and 500 alternating full previews per implementation for each template. Rendered outputs are exactly equal. Median latency fell 6–10% in this local run, but p95 was mixed, including a regression for the Node library. The decision is justified by deterministic allocation reduction and handling partial reads, not a throughput or SOTA superiority claim. No stale-template cache was introduced. A new regression test modifies a template between previews and replaces it with a symlink; both are rejected.
